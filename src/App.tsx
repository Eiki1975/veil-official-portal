...
function GallerySection() {
  ...
  return <section className="section gallery-section" id="gallery"><SectionTitle eyebrow="VISUAL RECORDS" title="GALLERY" copy="ステージの外で、彼女たちを知っていくための記録。" />
    <div className="gallery-collection">{galleryGroups.map(group => <section className="gallery-chapter" key={group.id} aria-labelledby={`gallery-${group.id}`}><header className="gallery-chapter-header"><p className="eyebrow">{group.eyebrow}</p><h3 id={`gallery-${group.id}`}>{group.title}</h3><p>{group.copy}</p></header><div className={`gallery-grid gallery-grid-${group.id}`}>{group.items.map(g => <button type="button" className="gallery-item" key={g.image} onClick={() => { setActive(g); track("gallery_image_click", g.category); }}><img src={assetUrl(g.image)} alt={g.alt} width="1536" height="1024" loading="lazy" /><span className="gallery-item-copy"><span className="gallery-category">{g.category}</span><span className="gallery-item-title">{g.title}</span><span className="gallery-caption">{g.caption}</span></span></button>)}</div></section>)}</div>
    ...
    {active && <div className="lightbox" role="dialog" aria-modal="true" aria-label={active.alt} onClick={() => setActive(null)}><button aria-label="閉じる"><X /></button><img src={assetUrl(active.image)} alt={active.alt} /></div>}
  </section>;
}
